...
const checkFunc = function (func) {
    if (typeof func !== 'function') {
        throw new Error('Пераметр не является функцией');
    }
}
...
const createLogger = function () {
    const obj = {
        ...
        print: function () {
            let sonArr = callHistory.slice();
            callHistory.length = 0;
            return sonArr;
        },

        call: function (func, ...pars) {
            checkFunc(func);

            let rez = func(pars[0], pars[1]);//

            callHistory.push({
                name: func.name,
                in:   pars.filter(element => element !== undefined),
                out:  rez,
            });
            return rez;
         }
